...
- **Title keyword color**: The slide title must have its most important keyword or phrase wrapped in a `<span>` with the theme's accent color (e.g., `text-blue-600`). The rest of the title stays in the default `text-gray-900`. This ensures the key concept visually pops against the surrounding text. Example: `MPF <span className="text-blue-600">60-Day</span> Enrolment Rule`.
- **Accent contrast rule**: The accent color on the title keyword MUST be visually distinct from `text-gray-900`. Never use dark neutrals like `text-slate-700`, `text-gray-700`, or `text-zinc-700` as accents — they are too close to the default title color and will not stand out. Always use a saturated color (e.g., `text-blue-600`, `text-indigo-600`, `text-emerald-600`).
