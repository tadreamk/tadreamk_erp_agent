...
Stage 5 - [Staff] Optional applicant-visible comment thread
  ...
  │   Entity type is job_application_workflow (workflow id), not job_application
  │
  ▼
...
Before calling the API, read relevant specs under `.claude.prod/api_doc/` (e.g. `job_application/`, `exercises/`, `comments/`).

## Auth and transport

- Load `WEBAPP_ACCESS_TOKEN` from project `.env`.
...
1. **Define the cohort** — e.g. paginate `GET /job-application?status=submitted` (or the user’s explicit `workflow_id` list). Only rows you intend to assign in this run count toward the check.
...
3. **Overview pass** — for each target `workflow_id`:
   - `GET /job-application/{workflow_id}` — if not assigning this status (e.g. already `exercise_assigned`), exclude from the *missing-recommendation* report or skip per your rules.
   - `GET /job-application/{workflow_id}/notes` — using the same parsing rules as **Internal recommendations**, decide if at least one note yields a **slug** present in `active-list`.
...
- Assignment body needs **`exercise_id` (UUID)**, not slug: `POST /job-application/{workflow_id}/assign-exercise` with `{"exercise_id":"<uuid>"}`.

## Internal recommendations (staff-only)

- Staff notes: `GET` / `POST /job-application/{workflow_id}/notes` (not visible to candidates per docs).
...
1. `GET /job-application/{workflow_id}` — if status is not `submitted`, skip unless you intentionally re-assign (normally stop at `exercise_assigned`).
2. `GET /job-application/{workflow_id}/notes` — derive **slug** from the exercise-suggestion note (same rules as pre-flight).
3. Map slug → UUID via cached `active-list`.
4. `POST /job-application/{workflow_id}/assign-exercise` with `{"exercise_id": uuid}`.
...
The UI thread for candidates is tied to comment entity type **`job_application_workflow`** (workflow id), not `job_application`.

1. `POST /comments/job_application_workflow/{workflow_id}` body `{"content":"..."}` (201).
...
- **Stale JSON snapshots** (e.g. `collected/job_applications/workflows_submitted.json`) omit **new** `submitted` workflows; refresh with paginated `GET /job-application?status=submitted` before bulk runs.
...
To refresh a merged snapshot: paginate `status=submitted`, then per id `GET /job-application/{id}` for contact, applications[], earliest `applied_at`, and write one JSON file under `collected/` only when the user wants a saved artifact.
