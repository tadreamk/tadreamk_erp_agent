...
Use for **TadReamk ERP** interview planning and **single-shot** proposal to candidates: discover workflows, build a slot grid (e.g. 30-minute blocks), and call **`POST /job-application/{workflow_id}/interview-propose`** only when appropriate.
...
Stage 1 - [Staff] Discover workflows
  │
  │   GET /job-application?status=exercise_submitted (and/or bookmark=true)
  │   GET /job-application/{workflow_id} for full profile when needed
  │
  ▼
Stage 2 - [Staff] Build slot table
  │
  │   Names, workflow_id, HKT blocks → slots[] as UTC ISO (+00:00)
...
Stage 3 - [Staff] Preflight per workflow before interview-propose
  ...
  │
  └──[Safe to propose]──> POST /interview-propose {"slots":[...]}
  │
...
- Contracts: read `.claude.prod/api_doc/job_application/` (especially `get_job-application.md`, `get_job-application_{workflow_id}.md`, `post_job-application_{workflow_id}_interview-propose.md`)

## Discovery

1. **Submitted exercise, not yet moved on:** `GET /job-application?status=exercise_submitted&limit=100`  
   Returns `workflows[]` with `id`, `candidate_name`, `username`, `status`, `position_titles`, `updated_at`.

2. **Starred (bookmark) candidates:** `GET /job-application?bookmark=true&limit=100`

3. **Full applicant profile (email, phone, resume, cover letter, assigned exercise, applications):** `GET /job-application/{workflow_id}`

Note: `GET /job-application/admin/kanban` may return empty/`No access` for some tokens; prefer list/detail endpoints above.

## Times and timezones

- Send **`slots` as UTC** ISO datetimes (e.g. `2026-05-08T01:00:00+00:00`). Align with `post_job-application_{workflow_id}_interview-propose.md`.
- **HKT = UTC+8.** Example (same calendar day): 09:00 HKT → `T01:00:00+00:00`; 14:30 HKT → `T06:30:00+00:00`.
- Request body: `{"slots": ["<utc-1>", ...]}`.

## Propose slots to applicants (email)

`POST /job-application/{workflow_id}/interview-propose` updates workflow state and **emails the candidate** with the proposed slot(s).
...
1. `GET /job-application/{workflow_id}`
2. If `status` is already `interview_proposed` **`and`** `interview_slots_proposed` already matches the slot(s) you intend → **do not** call `interview-propose` again. Tell the user the ERP already holds the proposal and the first mail was triggered when it was first proposed.
...
- Bookmarks: `PUT /job-application/{workflow_id}/bookmark`
